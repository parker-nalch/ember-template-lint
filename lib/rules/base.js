...
module.exports = class Base {
  constructor(options) {
    ...
    this._useFullFilePath = options.useFullFilePath;
  ...
  log(result) {
    ...
    if (this.filePath) {
      defaults.filePath = this._useFullFilePath ? [this.workingDir, this.filePath].join('/') : this.filePath;
    }
    ...
    // perform the node prop expansion only if those properties don't exist in result
    if (result.node) {
